...
'SUMS','BURP','ZINC','LENT','HOTS','SLAY','MAID','EYED','HOST','NANO','SHOT','THUS','REND','FADE','SHUT'];
...
function update_hs(){
	...
	//alert(score+' ' +highscore);	
	if(Math.sqrt(highscore)>Math.sqrt(score)){
		//alert(score+' ' +highscore);
		//alert("you are away");
		exit();
	}else{
		//alert("we are here");
		//alert(score+' ' +highscore);
		var ajax = ajaxObj("POST", "word.php");
		ajax.onreadystatechange = function() {
			if(ajaxReturn(ajax) == true) {
				_('highscore').innerHTML = score;
			}
		}
...
function checkhighscore(){
	var score = document.getElementById("score").innerHTML;
	var highscore = document.getElementById("highscore").innerHTML;
	if(Math.sqrt(score) == Math.sqrt(highscore)){
		alert("Congratulation!!! YOU SCORED A HIGH SCORE");
	}
}
</script>
<script>
